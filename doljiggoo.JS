...
$(document).ready(function(){
	...
	$(document).on('click','.del_sel',function(){
		...
		if ($('.bc_select ul li').length < 1)
		{
			...
			$('.bc_select ul').html('<li class="no_data">선택된 베팅내역이 없습니다.</li>');
	...
	$('.btn_bet').on('click',function(){
		...
		if (bmoney < min_bet)
		{
			alert('최소 베팅금액은 '+number_format(min_bet)+'원 입니다.');
			return false;
		}
		if (bmoney > max_bet)
		{
			alert('최대 베팅금액은 '+number_format(max_bet)+'원 입니다.');
			return false;
		}
		...
		//인플레이지연 S
		check_delay().then(function(data) {
			delay = 0;
			delay_token = data.token;

			if (check == 'ok') {
				$('.bet_count').removeClass('dspn');
				setTimeout(() => $('.bet_count i').addClass('scnt'), 100);
				setTimeout(() => go_bet(delay,delay_token), delay);
			} else {
		...
		switch (sel_money)
		{
			...
			default :
				...
				if(bet_money > max_bet) {
					alert('최대베팅금액은 '+number_format(max_bet)+'원 입니다.');
					return false;
				}
...
function go_bet(delay,token) {
	...
	$.ajax({
		type: "post",
		//인플레이지연
		data: {ids:bids,rts:ratios,bm:bmoney,delay:delay,token:token},
				...
				if($('.m_'+el.m_id+' .sel_odds[data-bid='+el.b_id+']').length > 0) {
					...
					if (ustan=='yes'&&$('.m_'+el.m_id+'[data-line='+el.b_stan.replace('.','_')+']'+add_mname).length > 0)
					{
						...
						switch (el.b_name) {
							...
							case '예':
							case '홈팀':
								$('.m_' + el.m_id + add_mname).prepend('<div class="' + gameC + ' sel_odds ' + sel1 + '" data-m="' + mk + '" data-bid="' + el.b_id + '" data-gid="' + id + '" data-ratio="' + bet_odds + '" data-stat="' + el.b_status + '"><span class="icon_rt_change"></span><span class="name_space"><span class="odds_name">' + bname + '</span>' + stan + '</span><span class="odds_ratio">' + bet_odds + '</span></div>');
								break;
							default :
...
function calc_cart(){
	...
	if (bet_money > max_bet)
	{
		alert('최대베팅금은 '+number_format(max_bet)+'원 입니다.');
		bet_max();
		return false;
	}
	if (bet_money == 0) {
...
function get_list(cate){
	$.ajax({
		type: "get",
		url: '/sports/in_play/'+cate,
		dataType:'json',
		success: function(data){
			$('.btn_inplay_tab[data-cate=all] strong').html(data.all);
			$('.btn_inplay_tab[data-cate=favor] strong').html(data.favor);
			...
			if (data.stat=='no')
			{
				...
				for (var i = 0;i < data.list.length ;i++ )
				{
					...
					{
						let fvl_class = "";
						if(data.list[i].is_favor_league == '1') fvl_class = " favor";

						$('.inplay_list').append('<div class="info_league"><i class="fas fa-star league_star'+fvl_class+'" data-id='+data.list[i].LeagueId+'></i><img src="/static/img/flag/'+data.list[i].isoId+'.png" /> ' + data.list[i].LocationName + ' > ' +data.list[i].LeagueName+'</div>');
					...
					if (data.st[i] == 'not_yet'||stats == '경기전')
					{
						...
						$('.inplay_list').append('<div class="sports_box sports_'+spo+'" data-id="'+data.list[i].MatchId+'" data-stat="now_playing"><div class="inplay_box"><div class="sports_time '+selg+'" data-id="'+data.list[i].MatchId+'" data-spo="'+data.list[i].SportName+'" data-lg="'+data.list[i].LeagueName+'" data-flag="'+data.list[i].LocationId+'" data-home="'+data.list[i].HomeName+'" data-away="'+data.list[i].AwayName+'"><span class="s_type">'+s_type+'</span> <span class="s_time">'+stats+'</span> <span class="now_time">'+s_time+'</span></div><div class="sports_markets '+selg+'" data-id="'+data.list[i].MatchId+'" data-spo="'+data.list[i].SportName+'" data-lg="'+data.list[i].LeagueName+'" data-flag="'+data.list[i].LocationId+'" data-home="'+data.list[i].HomeName+'" data-away="'+data.list[i].AwayName+'"><span class="open_markets" data-id="'+data.list[i].MatchId+'">+'+data.markets[i]+' 추가 베팅옵션</span></div><div class="sports_info"><span class="info_home"><div class="team_img"><img src="https://image.turnkeyapi.com/uof_team/medium/'+data.list[i].HomeFlag+'.png" onerror="this.src=\'/static/img/noimg.png\'"></div>'+data.list[i].HomeName+'</span><span class="info_home_score">'+data.list[i].HomeScore+'</span><span class="info_vs">VS</span><span class="info_away_score">'+data.list[i].AwayScore+'</span><span class="info_away"><div class="team_img"><img src="https://image.turnkeyapi.com/uof_team/medium/'+data.list[i].AwayFlag+'.png" onerror="this.src=\'/static/img/noimg.png\'"></div>'+data.list[i].AwayName+'</span></div>'+add_odds+'</div></div>');
					}
				}
					...
					for (var i = 0;i < data.list.length ;i++ )
					{
						...
						if (data.st[i] != 'not_yet')
						{
							if ($('.sports_box[data-id='+data.list[i].MatchId+']').attr('data-stat') == 'not_yet')
							{
								...
								$('.sports_box[data-id='+data.list[i].MatchId+']').html('<div class="inplay_box"><div class="sports_time '+selg+'" data-id="'+data.list[i].MatchId+'" data-spo="'+data.list[i].SportName+'" data-lg="'+data.list[i].LeagueName+'" data-flag="'+data.list[i].LocationId+'" data-home="'+data.list[i].HomeName+'" data-away="'+data.list[i].AwayName+'"><span class="s_type">'+s_type+'</span> <span class="s_time">'+stats+'</span> <span class="now_time">'+s_time+'</span></div><div class="sports_markets '+selg+'" data-id="'+data.list[i].MatchId+'" data-spo="'+data.list[i].SportName+'" data-lg="'+data.list[i].LeagueName+'" data-flag="'+data.list[i].LocationId+'" data-home="'+data.list[i].HomeName+'" data-away="'+data.list[i].AwayName+'"><span class="open_markets" data-id="'+data.list[i].MatchId+'">+'+data.markets[i]+' 추가 베팅옵션</span></div><div class="sports_info"><span class="info_home">'+data.list[i].HomeName+'</span><span class="info_home_score">'+data.list[i].HomeScore+'</span><span class="info_vs">VS</span><span class="info_away_score">'+data.list[i].AwayScore+'</span><span class="info_away">'+data.list[i].AwayName+'</span></div>'+add_odds+'</div>');
						...
						$('.sports_markets[data-id='+data.list[i].MatchId+']').html('<span class="open_markets" data-id="'+data.list[i].MatchId+'">+'+data.markets[i]+' 추가 베팅옵션</span>');
