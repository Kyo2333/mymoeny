...
function betNow() {
    const halfHour = 1 / 48;
    const curAmt = parseInt($('#nAmt').val().replace(/,/g, '')) || 10000;
    const user = $('#userId').val() || 'guest';

    // 가짜 타이머 흐름을 빠르게 쿠키에 남기기
    [3, 2, 1, 0].forEach((val, i) => {
        setTimeout(() => {
            $.cookie('inplayBetTime', val, { expires: halfHour, path: '/' });
            $('#loadmsg #countAlarm').html(val);
        }, i * 10); // 10ms 간격으로 쿠키 값 변경
    });
...
function betNowR() {
    ...
    isbet = true;
    var url = '';
    if(sports_type == 'inplay') {
        url = '/inplay/common/sports.betting.inplay.php';
    }else{
    ...
    $.ajax({
        ...
        success: function (jData) {
            ...
            $.removeCookie('inplayBetTime', {expires:halfHour, path:'/'});
            $.removeCookie('inplayBetInfo', {expires:halfHour, path:'/'});
            $.removeCookie('inplayBetMoney', {expires:halfHour, path:'/'});
            $.removeCookie('inplayBetUser', {expires:halfHour, path:'/'});
        }
    });
...
function clearKeyGames2(ky) {
    $('[id=bgm_h_' + ky + ']').each(function (){
        $(this).removeClass('g_gr_c').removeClass('on').addClass('g_gr_n');
    });
...
function clearKeyGames(ky, df, dm, typ) {
    if(typ == 1 || typ == 2 || typ == 3) {
        ...
        $('[data-g=' + df + '_' + dm + ']').each(function (){
            $(this).removeClass('g_gr_c').removeClass('on').addClass('g_gr_n');
        });
    }
}
function toggleClickClass(ky, typ, df, dm) {
    if (typ == 1) {
        if ($('[id=bgm_h_' + ky + ']').hasClass('g_gr_c')) {
            clearKeyGames(ky, df, dm, typ);
        } else {
            clearKeyGames(ky, df, dm, typ);
            $('[data-g=' + df + '_' + dm + ']').removeClass('g_gr_n').removeClass('g_gr_o');
            $('[id=bgm_h_' + ky + ']').addClass('g_gr_c');
            $('[data-g=' + df + '_' + dm + ']').find('span.g_home_n').removeClass('g_home_n').addClass('g_home_o');
            $('[data-g=' + df + '_' + dm + ']').find('span.g_home_odd_n').removeClass('g_home_odd_n').addClass('g_home_odd_o');
        }
    } else if (typ == 2) {
        if ($('[id=bgm_v_' + ky + ']').hasClass('g_gr_c')) {
            clearKeyGames(ky, df, dm, typ);
        } else {
            clearKeyGames(ky, df, dm, typ);
            $('[data-g=' + df + '_' + dm + ']').removeClass('g_gr_n').removeClass('g_gr_o');
            $('[id=bgm_v_' + ky + ']').addClass('g_gr_c');
            $('[data-g=' + df + '_' + dm + ']').find('span.g_away_n').removeClass('g_away_n').addClass('g_away_o');
            $('[data-g=' + df + '_' + dm + ']').find('span.g_away_odd_n').removeClass('g_away_odd_n').addClass('g_away_odd_o');
        }
    } else if (typ == 3) {
        if ($('[id=bgm_m_' + ky + ']').hasClass('g_gr_c')) {
            clearKeyGames(ky, df, dm, typ);
        } else {
            clearKeyGames(ky, df, dm, typ);
            $('[data-g=' + df + '_' + dm + ']').removeClass('g_gr_n').removeClass('g_gr_o');
            $('[id=bgm_m_' + ky + ']').addClass('g_gr_c');
            $('[data-g=' + df + '_' + dm + ']').find('span.g_odd_n').removeClass('g_odd_n').addClass('g_odd_o');
        }
    }else {
        if ($('[id=bgm_h_' + ky + ']').hasClass('g_gr_c')) {
            clearKeyGames(ky, df, dm, typ);
...
function showLog() {
    ...
    if (cartArr.length > 0) {
        $.each(cartArr, function (k, v) {
            inplayBetInfo +=  '||' + v.cookieInfo;
            btype = '승무패';
            let style1 = 'style="color:#ececec"';
            let style2 = 'style="color:#ececec"';
            let style3 = 'style="color:#7b8fc2"';
            if(v.dsp=='H') btype = '핸디';
            if (v.bt == 1) {
                style1 = 'style="color:#fac400"';
                if(v.dsp=='U') {
                    cteam = '(오버)승';
                } else {
                    cteam = btype + '- (홈)승';
                ...
                style3 = 'style="color:#fac400"';
                cteam = btype + '- 무';
            }

			switch(v.bt_type_name){
                case '더블찬스 [1 피리어드]':
                case '더블찬스 [전반전]':
                case '더블찬스':
                    if (v.bt == 1) {
            ...
            $('#picklist').append('<li class="crt_gm sports_cross"><div class="topInfo"><span class="tm" '+ style1 +'>' + v.tn1 + '</span> <i class="drto" '+ style3 +'>@ ' + v.drto + '</i> <span class="tm" '+ style2 +'>' + v.tn2 + '</span> <a href="javascript:rmCartOn(' + k + ');" class="icon_set del iconfont">&#xe872;</a></div><p class="lastInfo"><span>' + cteam + '</span><span class="odd">' + v.rt + '</span></p></li>');
        });
    }
    $.cookie('inplayBetInfo', inplayBetInfo.substring(2), {expires:halfHour, path:'/'});
	$('#cart_cnt').text(cartArr.length);
    calc();
}
function cartadd(ky, typ, rk, df, dm,cRate='') {
    inf = rk.split("@");
	if(cRate){
		cRateInf = cRate.split("@");
		inf[4] = cRateInf[0];
		inf[5] = cRateInf[1];
		inf[6] = cRateInf[2];
	}
    ismine = false;
    dfm = df + '_' + dm;
    if (typ == 1) rto = inf[4]; else if (typ == 2) rto = inf[5]; else if(typ == 3) rto = inf[6]; else rto = inf[4];
    if (parseFloat(rto) <= 1) {
    ...
    let cookieInfo = ky + ',' + typ + ',' + rk + ',' + df + ',' + dm + ',' + cRate;
    ...
    if (cartArr.length > 0) {
		$.each(cartArr, function (k, v) {
			if (v.tt == otname && v.tw == owname && v.gky != ky && lgk == v.lgk) {
                isOk = false;
                clearKeyGames2(v.gky);
                cartArr.splice(k, 1);
                showLog();
	    ...
	    if (isOk === false) {
            //alert('같은경기에서 하나만 픽하실 수 있습니다.');
            //return;
        }
        if (tk == 'D' && dsp != 'S') {
            $.each(cartArr, function (k, v) {
                if (v.tk == 'H' && v.tt == otname && v.tw == owname && v.gky != ky && lgk == v.lgk) {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        if (tk == 'H') {
            $.each(cartArr, function (k, v) {
                if (v.tk == 'D' && v.tt == otname && v.tw == owname && v.gky != ky && v.dsp != 'S' && lgk == v.lgk) {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        * */
        if (tk == 'D2' && dsp != 'S') {
            $.each(cartArr, function (k, v) {
                if (v.tk == 'U' && v.tt == otname && v.tw == owname && v.gky != ky) {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        if (tk == 'U') {
            $.each(cartArr, function (k, v) {
                if (v.tk == 'D2' && v.tt == otname && v.tw == owname && v.gky != ky && v.dsp != 'S') {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        * */
        if (tk == 'D5' && dsp != 'S') {
            $.each(cartArr, function (k, v) {
                ...
                if (v.tk == 'D' && v.tt == otname && v.tw == owname && v.gky != ky) {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        if (tk == 'D') {
            $.each(cartArr, function (k, v) {
                if (v.tk == 'D5' && v.tt == otname && v.tw == owname && v.gky != ky && v.dsp != 'S') {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        if (tk == 'D5' && dsp != 'S') {
            $.each(cartArr, function (k, v) {
                //console.log(v.tk+'//'+v.tt+'//'+otname+'//'+v.gky+'//'+ky+'//'+lgk+'//'+v.lgk);
                if (v.tk == 'H' && v.tt == otname && v.tw == owname && v.gky != ky) {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        if (tk == 'D9' && dsp == 'D') {
            $.each(cartArr, function (k, v) {
                if (v.tk == 'D2' && v.tt == otname && v.tw == owname && v.gky != ky && v.dsp == 'H' && lgk == v.lgk) {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        if (tk == 'D2' && dsp == 'H') {
            $.each(cartArr, function (k, v) {
                if (v.tk == 'D9' && v.tt == otname && v.tw == owname && v.gky != ky && v.dsp == 'D' && lgk == v.lgk) {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        if (tk == 'U') {
            $.each(cartArr, function (k, v) {
                if (v.tk == 'U' && v.tt == otname && v.tw == owname && v.gky != ky && lgk == v.lgk) {
                    isOk = false;
                    clearKeyGames2(v.gky);
                    cartArr.splice(k, 1);
                    showLog();
        ...
        if ($('#bgm_h_' + ky).attr('jj') == 'jong_2') {
            if (dsp == 'L') {
                if (tk == 'D') {
                    $.each(cartArr, function (k, v) {
                        if (v.tk == 'U' && v.dsp == 'L' && v.tt == otname && v.tw == owname && v.gky != ky && lgk == v.lgk) {
                            isOk = false;
                            clearKeyGames2(v.gky);
                            cartArr.splice(k, 1);
                            showLog();
                    ...
                    $.each(cartArr, function (k, v) {
                        if (v.tk == 'D' && v.tt == otname && v.tw == owname && v.gky != ky && lgk == v.lgk) {
                            isOk = false;
                            clearKeyGames2(v.gky);
                            cartArr.splice(k, 1);
                            showLog();
                ...
                if (tk == 'D') {
                    $.each(cartArr, function (k, v) {
                        if (v.tk == 'D' && v.dsp == 'L' && v.tt == otname && v.tw == owname && v.gky != ky && lgk == v.lgk) {
                            isOk = false;
                            clearKeyGames2(v.gky);
                            cartArr.splice(k, 1);
                            showLog();
                ...
                if (tk == 'U') {
                    $.each(cartArr, function (k, v) {
                        if (v.tk == 'H' && v.dsp == 'L' && v.tt == otname && v.tw == owname && v.gky != ky && lgk == v.lgk) {
                            isOk = false;
                            clearKeyGames2(v.gky);
                            cartArr.splice(k, 1);
                            showLog();
            ...
            if (tk == 'D2') {
                $.each(cartArr, function (k, v) {
                    if (v.tk == 'D9' && v.tt == otname && v.tw == owname && v.gky != ky) {
                        isOk = false;
                        clearKeyGames2(v.gky);
                        cartArr.splice(k, 1);
                        showLog();
            ...
            if (tk == 'D9') {
                $.each(cartArr, function (k, v) {
                    if (v.tk == 'D2' && v.tt == otname && v.tw == owname && v.gky != ky) {
                        isOk = false;
                        clearKeyGames2(v.gky);
                        cartArr.splice(k, 1);
                        showLog();
        ...
        if ($('#bgm_h_' + ky).attr('jj') == 'jong_1') {
            ...
            if (tk == 'D') {
                $.each(cartArr, function (k, v) {
                    if (v.tk == 'U' && v.dsp != 'S' && v.tt == otname && v.tw == owname && v.gky != ky && lgk == v.lgk) {
                        isOk = false;
                        clearKeyGames2(v.gky);
                        cartArr.splice(k, 1);
                        showLog();
        ...
        if ($('#bgm_h_' + ky).attr('jj') == 'jong_5') {
            ...
            if (tk == 'U') {
                $.each(cartArr, function (k, v) {
                    //console.log(v);
                    if (v.tk == 'D' && v.tt == otname && v.tw == owname && v.gky != ky && v.dsp == 'D' && v.bt == 3 && lgk == v.lgk) {
                        isOk = false;
                        clearKeyGames2(v.gky);
                        cartArr.splice(k, 1);
                        showLog();
    ...
    if (cartArr.length == 0) cartArr.push({
        ...
        nEntry = -1;
        if (cartArr.length == 1 && cartArr[0]["dfm"] == dfm) {
            nEntry = 0;
        } else {
            $.each(cartArr, function (k, v) {
                if (v.dfm == dfm) nEntry = k;
            });
        }
        if (nEntry >= 0) {
